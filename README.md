...
5. Создайте заметки в папке `content/` и сделайте `git push` (после `git commit`, разумеется).  
6. Сайт будет автоматически опубликован по адресу `https://{user}.github.io/{repo}` примерно через минуту. Более подробную информацию вы можете увидеть во вкладке `Deployments` справа в вашем репозитории на сайте.
